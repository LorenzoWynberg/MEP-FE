...
export const IdentificationInputs = props => {
	...
	const [orderedNationalitiesOptions, setOrderedNationalitiesOptions] = useState([])
	const actions = useActions({
		...
		<Form>
			<FormGroup>
				{props.label ? (
					...
					<RequiredLabel optional={props.idType && props.idType.codigo === '04'}>
				...
				<InputContainer>
					<Input
						...
						disabled={!props.editable || idDisabled || disabledFields || props.avoidSearch}
						onPaste={handleIdPaste}
						onChange={handleId}
						invalid={props.errors.identificacion || showError}
					/>
					{props.loading && <Loader formLoader styles={{ width: '36px', height: '33px' }} />}
				</InputContainer>
			</FormGroup>
			<FormGroup>
				{props.label ? (
					...
					<RequiredLabel>
						{t('estudiantes>expediente>info_gen>info_gen>tipo_id', 'Tipo de identificación')}
					</RequiredLabel>
				)}
				<Select
					...
					onChange={data => {
						...
						if (data.codigo == '04') {
							setIdDisabled(true)
							const notCostaRica = nationalitiesOptions.filter(item => item.codigo !== '15')
							setOrderedNationalitiesOptions(notCostaRica)
							props.handleChange([{}, data], 'idType')
						} else if (data.codigo == '01') {
							setIdDisabled(false)
							const _costaRica = nationalitiesOptions.filter(item => item.codigo == '15')
				...
				/>
				<FormFeedbackSpan>
					{props.errors.tipoIdentificacionId || props.errors.TipoIdentificacionId}
				</FormFeedbackSpan>
			</FormGroup>
			<FormGroup>
				{props.label ? (
				...
				<Select
					...
					isDisabled={!props.editable || disabledFields || props.avoidSearch}
					onChange={nationality => {
						props.handleChange(nationality, 'nationalityId')
					}}
				/>
				<FormFeedbackSpan>{props.errors.nacionalidadId || props.errors.NacionalidadId}</FormFeedbackSpan>
			</FormGroup>
