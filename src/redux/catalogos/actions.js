...
import {
	CATALOGOS_PROFILES_LOAD,
	CATALOGOS_SECTIONS_LOAD,
	CATALOGOS_ERROR,
	CATALOGOS_LOAD,
	CATALOGOS_CREATE,
	CATALOGOS_SET_ACTIVE,
	CATALOGOS_CHANGE_STATE
} from './catalogosTypes'
...
export const saveCatalogos = data => async dispatch => {
	try {
		const response = await axios.post(
			`${envVariables.BACKEND_URL}/api/Areas/GestorCatalogos/TipoCatalogo/Create`,
			data
		)
		if (response.data.error) {
...
export const updateCatalogos = data => async dispatch => {
	try {
		const response = await axios.put(
			`${envVariables.BACKEND_URL}/api/Areas/GestorCatalogos/TipoCatalogo/Update`,
			data
		)
		if (response.data.error) {
			dispatch(catalogosError(response.data.message))
...
export const updateStateCatalogos = (id, state) => async dispatch => {
	try {
		const response = await axios.put(
			`${envVariables.BACKEND_URL}/api/Areas/GestorCatalogos/TipoCatalogo/UpdateEstadoTipoCatalogo/${id}/${state}`
		)
		if (response.data.error) {
			dispatch(catalogosError(response.data.message))
			return { data: { message: response.data.message, error: true } }
...
export const canDeactivateTipoCatalogo = async tipoCatalogoId => {
	try {
		const response = await axios.get(
			`${envVariables.BACKEND_URL}/api/Areas/GestorCatalogos/TipoCatalogo/CanDeactivateTipoCatalogo/${tipoCatalogoId}`
		)
