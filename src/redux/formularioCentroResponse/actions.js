...
export const CreateNewFormResponse = async data => {
	try {
		const response = await axios.post(formCentroUrl, data)
...
export const UpdateFormResponse = async data => {
	try {
		const response = await axios.put(formCentroUrl, data)
		return { error: false, data: response.data }
	} catch (e) {
		return { error: true }
	}
...
export const GetFormResponseById = async id => {
	const response = await axios.get(`${formCentroUrl}GetById/${id}`)

	return response.data
}

export const ObtenerInfoCatalogos = async () => {
	const response = await axios.get(`${envVariables.BACKEND_URL}/api/ServicioComunal/ObtenerInfoCatalogos`)
...
export const GetResponseByInstitutionAndFormName = async (institutionId, formName, first = true) => {
	const response = await axios.get(`${formCentroUrl}GetAllByInstitucionAndFormName/${institutionId}/${formName}`)
...
// Obtiene el resultado de un determinado form e insittucion {para forms con respuesta unica}
export const GetServicioComunalInfoById = (idServicioComunal, idInstitucion) => async dispatch => {
	try {
		const response = await axios.get(
			`${envVariables.BACKEND_URL}/api/ServicioComunal/GetServicioComunalInfoById/${idServicioComunal}/${idInstitucion}`
		)
...
// Obtiene el resultado de un determinado form e insittucion {para forms con respuesta unica}
export const GetResponseByInstitutionAndFormNameUsingRedux =
	(institutionId, formName, reducerAction, first = true) =>
	async dispatch => {
		try {
			const response = await axios.get(
...
export const getlocationElementById = async (type, id) => {
	const response = await axios.get(`${envVariables.BACKEND_URL}/api/${type}/GetById/${id}`)
	return response.data || {}
}
...
export const DeleteFormResponses = async ids => {
	const response = await axios.delete(formCentroUrl, { data: ids })
	return response.data
}

// Lista de las soluciones paginadas
export const GetFormResponsesByInstitutionAndFormNamePaginated = async (
	institutionId,
	formName,
	page = 1,
	itemsByPage = 10
) => {
	const response = await axios.get(`${formCentroUrl}paginated/${institutionId}/${formName}/${page}/${itemsByPage}`)
