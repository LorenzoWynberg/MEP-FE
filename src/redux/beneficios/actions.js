...
export const GetSubsidiosMEP =
	(identidad, pagina, cantidad) => async (dispatch) => {
	  try {
	    const response = await axios.get(
...
export const editSubsidioBody = (data) => async (dispatch) => {
  try {
    const response = await axios.put(`${envVariables.BACKEND_URL}/api/ExpedienteEstudiante/Subsidio`,
      data)
    if (response.data.error) {
      dispatch(error(response.data))
...
export const addSubsidio = (data) => async (dispatch) => {
  try {
    const response = await axios.post(
			`${envVariables.BACKEND_URL}/api/ExpedienteEstudiante/Subsidio`,
			data
    )
    if (response.data.error) {
      dispatch(error(response.data))
...
export const GetSubsidiosFilterMEP =
	(identidad, filterText, pagina, cantidad) => async (dispatch) => {
	  try {
	    const response = await axios.get(
