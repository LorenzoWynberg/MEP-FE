...
const InstitutionSwitch = () => {
  ...
  const state = useSelector<any, any>((store) => {
    return {
      ...
      isRoleDirectorOrRoleSupervisor:
        store.authUser.currentRoleOrganizacion.accessRole.nivelAccesoId > 1
    }
  })
  ...
  if (
    state.isRoleDirectorOrRoleSupervisor &&
    state.currentInstitution?.id &&
    state.currentInstitution?.id != -1
  ) {
    return (
      <>
        <Marco onClick={() => {
