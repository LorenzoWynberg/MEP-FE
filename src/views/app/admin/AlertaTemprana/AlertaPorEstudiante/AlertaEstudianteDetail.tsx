...
import { getAlertById, getAlertByStudent, createAlertStepByStudent, closeAlertStudent } from '../../../../../redux/alertaTemprana/actionStudent'
...
const AlertaEstudianteDetail: React.FC<IProps> = (props) => {
  ...
  React.useEffect(() => {
    const datos = state.currentAlert?.pasos && state.currentAlert.pasos.map((paso, i) => {
      const isActive = state.currentAlertStudent.pasos && state.currentAlertStudent.pasos.length > 0 && state.currentAlertStudent.pasos[i]
      return {
        ...paso,
  ...
  const handleSave = async (values: string) => {
    ...
    setLoading(true)
    await actions.createAlertStepByStudent(props.currentStudent.alertaEstudianteId, data)
    setLoading(false)
  ...
  const handleCloseAlert = async (description: string) => {
    try {
      ...
      const res = await actions.closeAlertStudent(props.currentStudent.alertaEstudianteId, { razonDeCierre: description, recursosId: [] })
      if (!res.error) {
        swal({
  ...
  return (
    <Wrapper key={key}>
      ...
      <Card>
                      ...
                      ? <List>
                        {
                            pasos.map((step: any, i: number) => {
                              const totalFiles = state.currentAlertStudent && state.currentAlertStudent?.pasos && state.currentAlertStudent?.pasos.find(obser => obser.paso.numeroPaso == step.numeroPaso)
                              return (
                                <Item key={i}>
                                  <ItemInfo>
                                    <input type='checkbox' name='step' disabled={props.currentStudent.cerrada} checked={step.completado} onClick={() => handleStep(step, i)} />
                                  ...
                                  </ItemInfo>
                                  {
                                            step.completado && Object.keys(state.currentAlertStudent).length > 0
                                              ? <ItemResources>
                                                <ItemObservations>
                                                  <Button className='mr-1' size='sm' color='primary' outline onClick={() => handleAddObservations(step, i)} disabled={state.currentAlertStudent.cerrada}>Agregar nueva observación</Button>
                                                  <Button size='sm' color='primary' onClick={() => handleViewObservations(step, i)}>Ver observaciones</Button>
                                                </ItemObservations>
                                                <ItemUploads>
                                                  <Upload>
                                                    <FormGroup className='fied-upload'>
                                                      <div className='fileinput-buttons'>
                                                        <IconAdd />
                                                        <Button className='mr-1' size='sm' color='primary' outline disabled={state.currentAlertStudent.cerrada}>Subir Archivo</Button>
                                                        <input
                                                          accept='pdf/*'
                                                          id='profile-pic-id'
                                                          type='file'
        ...
        {
                    !props.currentStudent.cerrada ? <Button color='primary' onClick={handleSaveProcess}>Guardar Procesos</Button> : null
                }

        {
                    props.currentStudent.cerrada
                      ? <ActionLink color='#d1d1d1' className='ml-2' outline>Cerrar Alerta</ActionLink>
                      : <Button color='primary' className='ml-2' onClick={() => setCloseModal(!closeModal)} disabled={props.currentStudent.cerrada} outline>Cerrar Alerta</Button>
                }
        ...
        {loading ? <Loading><div className='single-loading' /></Loading> : null}
