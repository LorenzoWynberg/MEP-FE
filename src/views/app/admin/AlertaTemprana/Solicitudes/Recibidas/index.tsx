...
import { getAlertsByStatus, changeAlertToReview, changeAlertStatus, addCommentsToAlert } from '../../../../../../redux/alertaTemprana/actionRequests'
import { useSelector } from 'react-redux'
import { useActions } from 'Hooks/useActions'
import { Paginationtype } from '../../../../../../types/pagination'
import swal from 'sweetalert'

import AgregarObservacion from '../../modals/AgregarObservacion'
...
const AlertasRecibidas: React.FC<AlertaProps> = (props) => {
  const [loading, setLoading] = React.useState<boolean>(false)
  const [visible, setVisible] = React.useState<boolean>(false)
  const [detailModal, setDetailModal] = React.useState<boolean>(false)
  const [modalObservacion, setModalObservacion] = React.useState<boolean>(false)
  ...
  const actions = useActions({ getAlertsByStatus, changeAlertToReview, changeAlertStatus, addCommentsToAlert })

  const state = useSelector((store: IStore) => {
    return {
      received: store.alertaTemprana.alertsReceived,
      loading: store.alertaTemprana.loading
    }
  })
  ...
  const changeToReview = async (item: any) => {
    const res = await actions.changeAlertToReview(item.id)
    if (!res.error) {
      swal({
        title: 'Correcto',
  ...
  const actionRow = [
    ...
    {
      actionName: 'Agregar observación',
      actionFunction: (item) => {
        setCurrentAlert(item)
  ...
  const handleAddObservations = async (observations: string) => {
    setLoading(true)
    const data = {
      solicitudAlertaId: currentAlert.id,
      comentarios: [
        {
    ...
    const res = await actions.addCommentsToAlert(data)
  ...
  return (
    <Container>
      ...
      />
      <AgregarObservacion
        visible={modalObservacion}
        loading={loading}
        handleCancel={() => setModalObservacion(!modalObservacion)}
        handleSave={handleAddObservations}
      />
      {
                currentAlert !== null
                  ? <DetalleSolicitud
