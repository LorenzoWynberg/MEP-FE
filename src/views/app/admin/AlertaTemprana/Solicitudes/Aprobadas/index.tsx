...
const AlertasAprobadas: React.FC<AlertaProps> = (props) => {
  ...
  const actionRow = [
    ...
    {
      actionName: 'Crear alerta',
      actionFunction: (item) => {
        props.history.push({ pathname: '/admin/alerta-catalogo', search: '?action=creation', state: { newResource: true } })
      },
      actionDisplay: () => true
    },
    {
  ...
  const handleSave = async (observations: string) => {
    setLoading(true)
    const data = {
