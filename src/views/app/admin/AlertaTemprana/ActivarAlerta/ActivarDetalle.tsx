...
import { activeAlertStudent, getAlertById } from '../../../../../redux/alertaTemprana/actionStudent'
...
const ActivarDetalle: React.FC<IProps> = (props) => {
  const actions = useActions({ uploadFicha, activeAlertStudent, getAlertById })
  ...
  const fetchAlert = async () => {
    await actions.getAlertById(props.currentAlert.id)
  }
  ...
  return (
    <Wrapper>
      <Row>
        <Col md={6}>
          <Card>
            <Title>Descripción - {state.currentAlert.nombre}</Title>
            <Content>
              <Description>{state.currentAlert.descripcion}</Description>
            </Content>
            <ActionsInfo>
              <Button size='sm' className='d-flex align-items-center mr-2' color='primary' outline onClick={() => handleOpenFile(state.currentAlert.fichaUrl)}><IconEye className='pr-1' /> Ver ficha completa</Button>
