...
const ModificarAlertaForm: React.FC<IProps> = (props) => {
  ...
  const {
    ...
    mode: 'onChange',
    defaultValues: {
      nombre: props.alerta.nombre,
      atencionInmediata: props.alerta.atencionInmediata ? 'Sí' : 'No',
      descripcion: props.alerta.descripcion,
      recordatorio: recordatorios.find(recordatorio => recordatorio.value === props.alerta.recordatorio),
      nivelPrioridad: props.alerta.nivelPrioridad === 0 ? '' : props.alerta.nivelPrioridad,
      esCompuesta: props.alerta.nivelPrioridad > 0 ? 'Sí' : ''
    }
  })

  React.useEffect(() => {
    const responsables = props.alerta.responsables.map(responsable => responsable.id)
    setSelectedLevels(responsables)
  }, [props.alerta])

  React.useEffect(() => {
    const currentContexto = props.alertsContext.find(context => context.id === props.alerta.contextoId)
    const currentDimension = props.alertsDimension.find(context => context.id === props.alerta.dimensionId)
    const currentPrio = prioridad.find(prio => prio.value === props.alerta.nivelPrioridad)
    setValue('contextId', currentContexto)
    setValue('DimensionId', currentDimension)
    setValue('nivelPrioridad', currentPrio)
  }, [props.alerta, props.alertsContext, props.alertsDimension])
  ...
  const onSubmit = (values: any) => {
    props.handleUpdateAlert(values, selectedLevels)
  }
  ...
  return (
    <Content>
      <Card className='bg-white__radius'>
        <CardTitle>Modificar alerta</CardTitle>
        <Form>
          <FormGroup>
            <Label>Nombre de la alerta</Label>
            <Input name='nombre' innerRef={register({ required: true })} readOnly autoComplete='off' />
...
export default ModificarAlertaForm
