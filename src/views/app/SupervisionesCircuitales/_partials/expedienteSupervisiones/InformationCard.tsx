...
const InformationCard = ({ data }) => {
  ...
  const state = useSelector((store: IState) => {
    return {
      currentCircuito: store.configuracion.expedienteSupervision || store.configuracion.currentCircuito
    }
  })

  useEffect(() => {
    const loadData = async () => {
      const location = JSON.parse(state.currentCircuito.ubicacionGeograficaJson)
    ...
    if (state.currentCircuito.ubicacionGeograficaJson) {
      loadData()
    }
          ...
          <Row>
            ...
            >
              <span className={`${classes.information}`}>
                {t('supervision_circ>expediente>supervision', 'Supervisión')}: {state.currentCircuito?.nombre}
              </span>
              <span className={`${classes.information}`}>
                {t('supervision_circ>expediente>codigo_mep', 'Código MEP')}: {state.currentCircuito?.codigo}
              </span>
              <span className={`${classes.information}`}>
                {t('supervision_circ>expediente>codigo_presupuestario', 'Código presupuestario')}:{' '}
                {state.currentCircuito?.codigoPresupuestario}
              </span>
            </Col>
            <Col
              xs='12'
