  ...
  useEffect(() => {
    if (isVerify) {
      ...
      data?.datos?.forEach((item) => {
        ...
        const selected = state.selects[typeCat[0].name].filter(
          (x) => x.id === item.elementoId
        )
        let _obj = {}
        ...
        <Row>
          ...
          {dataForm.tipoIdentificacionId != 4
            ? (<Col md={6}>
              <FormGroup>
                <Label>
                  {t("configuracion>centro_educativo>ver_centro_educativo>asignar_director>asignar_director>numero_identificacion", "Número de identificación")}
            ...
            : (<Col md={6}>
              <FormGroup>
                <Label>
                  {t('general>label>tipo_yisro','Tipo de YísRö')}
                </Label>
                <Input
                  name='tipoYisro'
                  autoComplete='off'
          ...
          <Col md={dataForm?.tipoIdentificacionId === 3 ? 12 : 6}>
            <FormGroup>
              <Label>{t("configuracion>centro_educativo>ver_centro_educativo>administracion_auxiliar>agregar>nacionalidad", "Nacionalidad")}</Label>
              <Input
                name='nationality'
                autoComplete='off'
                readOnly
          ...
          <Col md={6}>
            <FormGroup>
              <Label>{t("configuracion>centro_educativo>ver_centro_educativo>administracion_auxiliar>agregar>primer_apellido", "Primer apellido")}</Label>
              <Input
                name='primerApellido'
                autoComplete='off'
                readOnly
