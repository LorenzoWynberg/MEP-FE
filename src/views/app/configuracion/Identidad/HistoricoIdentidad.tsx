...
import SinIdentificacion from './modals/SinIdentidad'
import PreviewChange from './PreviewUserBitacora'
...
const HistoricoIdentidad: React.FC<IProps> = (props) => {
  ...
  const columns = [
    { column: 'fechaActualizacion', label: t('estudiantes>indentidad_per>historico_camb>fecha', 'Fecha del evento'), width: 30 },
    { column: 'nombreCompleto', label: t('estudiantes>indentidad_per>historico_camb>user', 'Usuario aplicó cambio'), width: 70 }
  ]
  ...
  const handleInputSearch = async (
    ...
    const { data } = await actions.getIdentificacionPersona(search)
    setIdentidad(data)
    if (Object.keys(data).length === 0) {
      ...
      if (!res.error) {
        ...
        showNotification(
          'error',
          t('general>error', 'Oops. Algo ha salido mal. Por favor intentelo luego')
        )
      }
    }
  }
  ...
  return (
    <Wrapper>
      ...
      <Title>{t('estudiantes>indentidad_per>historico_camb>titulo', 'Histórico de cambios a la identidad')}</Title>
      <Form>
        <FormRow>
          <FormInline>
          ...
          <Search>
            <Button onClick={handleInputSearch}>{t('general>buscar', 'Buscar')}</Button>
          </Search>
        </FormRow>
      </Form>
      {identidad ? (
        <Row>
          <Col md={6}>
            <Card className='bg-white__radius'>
              <CardTitle>{t('estudiantes>indentidad_per>historico_camb>bitacora', 'Bitácora de cambios')}</CardTitle>
              {data.length > 0 && (
                <p style={{ marginTop: '5px' }}>
                  {t('estudiantes>indentidad_per>historico_camb>mensaje', 'Seleccione la fila de información para visualizar la ficha de datos que muestra los cambios realizados en la identidad de la persona.')}
                </p>
              )}
              <ContentPicker>
                <DatePicker
          ...
          {previewUser !== null
            ? (
              <Col md={6}>
                <PreviewChange
                  key={random}
                  title={t('estudiantes>indentidad_per>historico_camb>datos_solicitados', 'Datos solicitados')}
                  disabled
                  user={previewUser}
                />
              </Col>
...
export default HistoricoIdentidad
