...
const MensajeView = (props) => {
  ...
  const state = useSelector(store => {
    return store.comunicados
  })
  const [reply, setReply] = useState(false)
  const [resend, setReSend] = useState(false)
  const [areColored, setColor] = useState(false)
  const [dropdownOpen, setDropdownOpen] = useState(false)
  const [printer, setPrinter] = React.useState<boolean>(false)

  useEffect(() => {
    setColor(props.data.esFavorito)
            ...
            <Col md='3'>
              <div style={{ float: 'right' }}>
                ...
                <i
                  ...
                  style={areColored ? { color: '#145388' } : { color: 'grey' }}
                />
                <i
                  id='responder'
        ...
        <Col xs='12'>
          <div style={{ width: '100%' }}>
            {data.etiquetaNombre
              ...
              : (
                  [...state.etiquetas, ...state.etiquetasPersonalizadas].map(
                    (el) => {
                      if (
                        el.id == data.etiquetaId ||
                    el.id == data.etiquetaPersonalizadaId
                      ) {
                        return (
                          <span
                            style={{
