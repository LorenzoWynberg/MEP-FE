...
const Buscador = (props) => {
  ...
  const getDataFilter = async (filterText) => {
    let response = null
    setLoading(true)
    response = await props.getStudentFilterExpediente(
      filterText,
      1,
      100,
    ...
    if (response.data.length === 0) {
      swal({
        icon: 'info',
        text: t('estudiantes>expediente>msj_no_encontrado', 'El estudiante no está matriculado en este centro educativo'),
        buttons: {
          ok: {
    ...
    if (response.data.length === 1) {
      if (response.data[0].fallecido) {
        swal({
          icon: 'info',
          text: t('estudiantes>expediente>msj_fallecido','La persona estudiante que está consultando se encuentra registrada como FALLECIDA, por lo cual hay funcionalidades en el sistema que solamente son de consulta.'),
          buttons: {
            ok: {
              text: t("general>aceptar", "Aceptar"),
              value: true
  ...
  return (
    <div>
      ...
      <div>
        <Colxx xxs='12'>
          <Breadcrumb
            heading={t('estudiantes>expediente>buscador>titulo', 'Expediente estudiante')}
            match={props.match}
            hidePath
          />
          <br />
