...
const TablaMep = (props) => {
  const { data, loading, handlePagination, handleSearch, totalRegistros } =
		props
    ...
    return [
      ...
      },
      {
        ...
        Cell: ({ _, row, data }) => {
          ...
          return (
            <div>
              {`${moment(fullRow.fechaInicio).format(
								'DD/MM/YYYY'
							)} - ${moment(fullRow.fechaFinal).format(
								'DD/MM/YYYY'
							)}`}
            </div>
          )
