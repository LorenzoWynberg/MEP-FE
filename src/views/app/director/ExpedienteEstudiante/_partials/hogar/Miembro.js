...
import { Row, Modal, ModalBody, ModalHeader, Form, Button } from 'reactstrap'
...
import InformacionContacto from './InformacionContactoRol.tsx'
...
const General = (props) => {
  ...
  useEffect(() => {
    const loadData = async () => {
      if (state.miembro.currentMember.id) {
        await actions.getFamilyMember(state.miembro.currentMember.id)
      }
      setLoading(false)
    }
    loadData()
  ...
  const sendData = async () => {
    await props.sendData(discapacidades)
  }
  ...
  const discapacidadesStore =
		props.selects[catalogsEnumObj.DISCAPACIDADES.name]
  ...
  useEffect(() => {
    setDiscapacidades(props.memberData.discapacidades)
  }, [props.memberData.discapacidades])
  return (
            ...
            <Row>
              <Colxx xxs='12' md='6' className='mt-5'>
                <MiembroPersonalDataForm
                  ...
                  discapacidades2={discapacidades}
