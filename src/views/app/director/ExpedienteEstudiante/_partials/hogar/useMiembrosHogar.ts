  ...
  switch (type) {
    ...
    case TYPES.SET_ALL_CATALOGS:
      return {
        ...
        relacionConEstudianteCatalog:
          payload.relacionConEstudianteCatalog,
...
const useMiembrosHogar = ({ setSnackbarContent, handleClick }) => {
  ...
  const reduxState = useSelector((store: any) => {
    return {
      ...
      estudianteId:
        store.expedienteEstudiantil.currentStudent.idEstudiante
    }
  })
  const clearForm = () => {
  ...
  const loadCatalogs = async () => {
    ...
    const { data: relacionConEstudiante } = await getCatalogs(14)(
      reduxDispatch
    )
    const { data: discapacidades } = await getCatalogs(15)(reduxDispatch)
    const { data: condicionesLaborales } = await getCatalogs(TIPO_CATALOGO_DATOS_ADICIONALES.CondicionLaboral)(reduxDispatch)
    const { data: escolaridades } = await getCatalogs(TIPO_CATALOGO_DATOS_ADICIONALES.Escolaridad)(reduxDispatch)
    const payload = {
      tipoIdentificacionCatalog:
        reduxState.tipoIdentificacion.map(mapeador),
      ...
      identidadGeneroCatalog:
        reduxState.identidadGenero.map(mapeador),
  ...
  const getRequest = (type: 'create' | 'update' = 'create') => {
    ...
    state.documentosEncargado.filter((el) => el?.action !== 'remove').forEach((i) => {
      formBody.append('documentosEncargado', i.blob)
    })
    state.documentosRepresentanteLegal.filter((el) => el?.action !== 'remove').forEach((i) => {
  ...
  const setFormValuesFromRequest = (result, catalogos) => {
    ...
    const newState: typeof initialState = {
      ...
      relacionConEstudiante: catalogos.relacionConEstudianteCatalog.find(i=>i.value == miembro.sb_elementosCatalogoId),
