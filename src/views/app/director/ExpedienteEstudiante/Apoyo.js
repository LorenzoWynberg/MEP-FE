...
const General = (props) => {
  ...
  const handleDeleteApoyo = async (id, categoryKeyName, category) => {
    swal({
      ...
      if (result) {
        return await actions.deleteApoyo(id, categoryKeyName, state.identification.data.id, state.apoyos[categoryKeyName].pageNumber, category)
      }
    })
  }
        ...
        return (
          <>
            {state.apoyos[storedValuesKey] &&
              <CollapseCard
                titulo={categoria.nombre} label={t('general>agregar', 'Agregar')} addItem={() => {
