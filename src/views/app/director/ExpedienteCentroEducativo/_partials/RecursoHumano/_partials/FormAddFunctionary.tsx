...
const getRolNombre = (obj) => {
  const roles = obj?.rolInfoData?.roles

  if (roles && roles.length > 0) {
    return roles[0].rolNombre
  } else {
    return 'No asignado'
  }
}

const FormAddFunctionary: React.FC<IProps> = (props) => {
  const {
    idValueRegister,
    idTypeRegister,
    onChangeId,
  ...
  console.log('TOTAL INFO', servantFounded)
    ...
    <FormStyled>
      ...
      {servantFounded && (
        <ServantFounded>
          ...
          <Label>Rol:</Label>
          <Input disabled value={getRolNombre(servantFounded)} />
        </ServantFounded>
      )}
      {notFound && (
        <ServantNotFound>
...
FormAddFunctionary.defaultProps = {
  notFound: false,
  servantFounded: null
}
export default FormAddFunctionary
