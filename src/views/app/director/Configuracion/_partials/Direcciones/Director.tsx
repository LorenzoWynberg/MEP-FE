...
import ModalUsuarios, { TIPO_USUARIO } from '../Centro/ModalUsuarios'
import colors from '../../../../../../assets/js/colors'
import { useSelector } from 'react-redux'
import useNotification from 'Hooks/useNotification'
import { useActions } from 'Hooks/useActions'
import { buscarDirectores, saveRegionalDirector, getRegionalDirector } from 'Redux/configuracion/actions'
import { CurrentRegional, Configuracion } from '../../../../../../types/configuracion'
...
const Director = (props: IProps) => {
  ...
  const actions = useActions({ buscarDirectores, saveRegionalDirector, getRegionalDirector, getEstadoFallecido })
  ...
  React.useEffect(() => {
    const fetch = async () => {
      const { data } = await actions.getRegionalDirector(props.currentRegional.id)
  ...
  const handleInputSearch = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const result = await actions.buscarDirectores({ pagina: 1, cantidad: 10, keyword: search || '1' })

    const fallecido = await getFallecido(result.data.entityList[0].identificacion)
    if (fallecido === true) {
      showNotification('error', 'La persona que usted eligió se encuentra fallecida, favor verificar.')
    }

    if (!result.error) {
      if (result.data.entityList.length === 0) {
        showNotification('error', 'El director que busca no cuenta con usuario en sistema')
        setModal(!modal)
      }

      if (result.data.entityList.length === 1) {
        const user = result.data.entityList.find((item: any) => item.identificacion == search)
  ...
  const onSubmit = async (values) => {
    try {
      ...
      setLoading(true)
      await actions.saveRegionalDirector(data)
      setLoading(false)
      showNotification('success', 'Se ha asignado el director correctamente')
  ...
  const getFallecido = async (filterText) => {
    const res = await actions.getEstadoFallecido(
      filterText,
      true
    )
    return res.data
  ...
  return (
    <Wrapper>
      ...
      <Card className='bg-white__radius'>
        <CardTitle>{t('configuración>direcciones_regionales>agregar>director>datos_del_director', 'Datos del director')}</CardTitle>
        <Form>
          <FormRow>
            <FormInline>
              <Label>{t('configuración>direcciones_regionales>agregar>director>identificacion', 'Identificación')} </Label>
              <Input type='text' value={search} onChange={handleChange} disabled={editable} />
            </FormInline>
            <Search>
              <Button onClick={handleInputSearch} opacity={editable} disabled={!!editable}>{t('general>buscar', 'Buscar')}</Button>
            </Search>
          </FormRow>
          <FormGroup>
            <Label>{t('configuración>direcciones_regionales>agregar>director>nombre', 'Nombre')}</Label>
            <Controller
              control={control}
              name='nombre'
              as={
          ...
          <FormGroup>
            ...
            <Input name='primerApellido' type='text' ref={register({ required: false })} />
          </FormGroup>
          <FormGroup>
            <Label>{t('configuración>direcciones_regionales>agregar>director>segundo_apellido', 'Segundo apellido *')}</Label>
            <Input name='segundoApellido' type='text' ref={register({ required: true })} disabled={false} />
          ...
          <FormGroup>
            ...
            <Input name='email' type='text' ref={register({ required: false })} disabled />
          </FormGroup>
        </Form>
        {loading ? <Loading><span className='single-loading' /></Loading> : null}
      </Card>
      <Actions>
        {
                    editable
                      ? <>
                        {hasEditAccess ? <ActionButton onClick={() => setEditable(!editable)}>{t('general>editar', 'Editar')}</ActionButton> : null}
                      </>
                      : <>
                        <BackButton onClick={handleCancel}>{t('general>cancelar', 'Cancelar')}</BackButton>
                        <ActionButton onClick={handleSubmit(onSubmit)}>{editable ? t('general>editar', 'Editar') : t('general>guardar', 'Guardar')}</ActionButton>
                        </>
                }
      </Actions>
      {modal &&
        <ModalUsuarios
          tipoUsuario={TIPO_USUARIO.DIRECTOR_REGIONAL}
          visible={modal}
...
const Form = styled.form`color: #000;`
...
const Label = styled.label`color: #000; display: block;`
