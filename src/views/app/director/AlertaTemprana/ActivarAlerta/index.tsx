...
import { getAlertStudents, getAlertStudentsPaginated } from '../../../../../redux/alertaTemprana/actionStudent'
...
const ActivarAlerta: React.FC<AlertaProps> = props => {
	...
	const actions = useActions({ getAlertStudents, getAlertStudentsPaginated, getAlertsByCatalog })
	...
	const fetchAlerts = async () => {
		setLoading(true)
		await actions.getAlertStudentsPaginated('', '', '', pagination.pagina, pagination.cantidad)
		setLoading(false)
	}

	React.useEffect(() => {
		if (props.currentStudent && Object.keys(props.currentStudent).length > 0) {
		...
		<Container>
			<Wrapper>
				...
				{props.currentAlert === null ? (
					<HTMLTable
						key={props.key}
						columns={props.type === 'students' ? columns : columnsAlerts}
						...
						actionRow={props.type === 'students' ? null : actionAlertRow}
						match={props.match}
						...
						handlePagination={(pageNumber: number, selectedPageSize: number) => {
							if (props.type === 'students') {
								actions.getAlertStudentsPaginated('', '', '', pageNumber, selectedPageSize)
							} else {
						...
						handleSearch={(
							...
							if (props.type === 'students') {
								...
								actions.getAlertsByCatalog(page, selectedPageSize, selectedColumn, searchValue)
							}
						}}
						...
						roundedStyle
						filterdSearch
						orderOptions={props.type === 'students' ? columns : columnsAlerts}
						readOnly
						hideMultipleOptions
						preferences
						buttonSearch
