...
const parseDatosToSelectValue = (datos, selects) => {
  ...
  datos.forEach((item) => {
    let typeCat = catalogsEnum.find((x) => x.id === item.catalogoId)
    let selected = selects[typeCat.name].find((x) => x.id === item.elementoId)
    let _obj = {}
...
const FormData: React.FC<IProps> = (props) => {
  ...
  const mapper = (data) => {
    let datos = parseDatosToSelectValue(data.datos, state.selects)
  ...
  const _onMatricular = async () => {
    ...
    let data = {
      ...
      numeroAyudaConectividad: numeroAyudaConectividad,
    }
    setLoading(true)
    let response = await onMatricular(data)
    if (response) {
      resetData()
    }
    setLoading(false)
