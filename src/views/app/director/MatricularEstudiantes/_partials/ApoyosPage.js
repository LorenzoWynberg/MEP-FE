...
import { getTiposApoyos, getDependenciasApoyos, getCategoriasApoyos, getApoyosByType, addApoyo, deleteApoyo, editApoyo, getDiscapacidades, saveDiscapacidades, getCondiciones, getResources, clearCurrentDiscapacidades } from '../../../../../redux/apoyos/actions'
...
const General = (props) => {
  ...
  const agregarApoyo = async (data, category, categoryKeyName) => {
    ...
    const _finded = _apoyosGuardados.find(x => {
      ...
      return x.tipoDeApoyoId === data.tipoDeApoyoId && _fechaFinStr === data.fechaFin && _fechaInicioStr === data.fechaInicio
    })
  ...
  const editarApoyo = async (data, category, categoryKeyName) => {
    ...
    const _finded = _apoyosGuardados.find(x => {
      const _fechaFinStr = x.fechaFin.substring(0, 10)
      const _fechaInicioStr = x.fechaInicio.substring(0, 10)
      return x.id !== data.id && x.tipoDeApoyoId === data.tipoDeApoyoId && _fechaFinStr === data.fechaFin && _fechaInicioStr === data.fechaInicio
    })
  ...
  const confirmarEliminar = async (data) => {
    if (dataEliminar !== null) {
      ...
      await actions.deleteApoyo(dataEliminar.id, dataEliminar.categoryKeyName, state.identification.data.id, state.apoyos[dataEliminar.categoryKeyName].pageNumber, dataEliminar.category)
  ...
  return (
    <>
      ...
      <h4>{t('estudiantes>matricula_estudiantil>matricular_estudiante>apoyos_educativos>apoyos_educativos', 'Apoyos educativos')}</h4>
      <br />
      <ApoyoEducativo
      ...
      {state.apoyos.categorias.map(categoria => {
        ...
        const totalRows = state.apoyos[storedValuesKey] !== undefined ? state.apoyos[storedValuesKey].entityList?.length : 0
        const _items = state.apoyos[storedValuesKey] !== undefined ? state.apoyos[storedValuesKey].entityList : []
        const orderedDescItems = _items === undefined ? [] : _items.sort((a, b) => parseInt(b.id) - parseInt(a.id))
          ...
          <>
            {state.apoyos[storedValuesKey] &&
              <CollapseCard
                ...
                titulo={categoria.nombre} label='Agregar' addItem={() => {
                  setAddItems({ ...addItems, [storedValuesKey]: true })
                }}
              >
                <Container>
                  {width > 800 && <ApoyoHeader
                  ...
                  {!state.apoyos[storedValuesKey].loading
                    ? <>
                      {orderedDescItems.map(apoyo => {
                        return (
                          <ApoyoItem
                            ...
                            tipos={state.apoyos.tipos.filter(tipo => tipo.categoriaApoyoId === categoria.id)}
                      ...
                      <Pagination
                        ...
                        totalPage={state.apoyos[storedValuesKey].totalCount % 5 === 0 ? state.apoyos[storedValuesKey].totalPages : state.apoyos[storedValuesKey].totalPages + 1}
                        onChangePage={async (i) => await actions.getApoyosByType(state.identification.data.id, i, 5, categoria)}
                      />
                    </>
