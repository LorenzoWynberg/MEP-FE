...
const StundentComponentSettings = ({
	components,
	setComponents,
	subject,
	selectedPeriod,
	...
	const updateComponents = async () => {
		...
		if (
			...
			response = await swal({
				title: t(
					'gestion_grupos>apoyo_curricular>eliminar_calificaciones',
					'Eliminar calificaciones'
				),
				text: t(
					'gestion_grupos>apoyo_curricular>eliminar_calificaciones_texto',
					'¡Este cambio eliminara todas las calificaciones de este periodo!'
				),
				icon: 'warning',
				...
				Cell: ({ row }) => {
					...
					return (
						<div className="d-flex justify-content-center align-items-center">
							...
							<Tooltip
								title={t(
									'general>tooltip>eliminar_subcomponente',
			...
			data: [
				...currentComponents.map((component, index) => {
					...
					return {
						...
						acciones: 'edit',
						value: component,
						escala: component?.escalaCalificacion ? 'edit' : 'add',
						noRequiereInstrumentos:
							component?.noRequiereInstrumentos,
						style: {
							backgroundColor:
								index % 2 !== 0 ? colors.opaqueGray : 'unset'
						},
						subRows
					}
				})
		...
		addInstrument: {
			...
			handleClick: async () => {
				...
				if (index !== -1) {
					...
					if (response) {
						// res = await actions.updateJSON('Componentes', newComponents, subject, selectedPeriod)
					}
					if (!res?.error) {
		...
		addEscala: {
			...
			handleClick: async () => {
				...
				if (index !== -1) {
					...
					const response = await updateComponents()
					if (response) {
						// await actions.updateJSON('Componentes', newComponents, subject, selectedPeriod)
					}
				}
				setOpenModal('')
				...
				<CardBody>
					{apoyoCurricular && (
						<div style={{ width: '15rem' }}>
							<Select
								placeholder={t(
									'general>placeholder>escoger_estudiante',
									'Escoger un estudiante'
								)}
								value={
									selectedStudent
										? {
												label: selectedStudent?.nombreCompleto,
					...
					<Table
						data={
							selectedPeriod === null || selectedStudent === null
								? []
								: tableState.data
						}
...
export default StundentComponentSettings
