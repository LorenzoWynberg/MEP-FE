...
const ReporteRegistroEstudiantes = ({ regresarEvent, type = 'conducta' } : { regresarEvent: any, type: 'conducta' | 'asistencia' }) => {
  ...
  const loadReportData = async (modeloOfertaId = 0, nivelOfertaId = 0, grupoId = 0) => {
    try {
      setLoader(true)
      const response = await axios.get<any>(
				`${envVariables.BACKEND_URL}/api/Areas/Reportes/ReportesGenerales/${ type === 'conducta' ? 'Incidencias' : 'Asistencias'}/${0}/${0}/${modeloOfertaId}/${nivelOfertaId}/${grupoId}`
      )
  ...
  const onShowReportEvent = (parameters) => {
    const { nivelOfertaId, nivelId, grupoId } = parameters

    // if (!nivelOfertaId) return
    
    setParameters(parameters)
    loadReportData(nivelId.value, nivelOfertaId.value, grupoId.value).then(() => {
      setState(1)
    })
  }
  const onExcelEvent = () => {
  ...
  return (
    <div>
      ...
      />
      {state === 0 && <Parameters showReportEvent={onShowReportEvent} reportLoader={loader} type={type} />}
