...
export class WebMapView extends React.Component {
  ...
  componentDidMount () {
    ...
    const self = this
    loadModules(['esri/Map', 'esri/views/MapView', 'esri/widgets/Search', 'esri/layers/FeatureLayer'], {
      css: true
      ...
      async function queryFeatureLayer (point, distance, spatialRelationship, featureL) {
        const query = {
          geometry: point,
          distance,
          spatialRelationship,
      ...
      this.view.on('click', async function (evt) {
        self.disableMap(evt, async () => {
          ...
          let ubicacionResponse = await self.queryFeatureLayer(evt.mapPoint, 1500, 'intersects', self.distritosLayer)
          ubicacionResponse = ubicacionResponse[0]?.attributes
          if (self.search.activeSource) {
            const geocoder = self.search.activeSource.locator // World geocode service
            const params = {
            ...
            geocoder.locationToAddress(params).then(
              ...
              function (err) {
                ...
                self.showPopup('No address found.', evt.mapPoint, ubicacionResponse)
              }
            )
          }
        })
  ...
  showPopup (address, pt, attributes) {
    ...
    this.props.setUbicacion({ canton: attributes?.NCANTON, provincia: attributes?.PROVINCIA, distrito: attributes?.NDISTRITO })
  }
