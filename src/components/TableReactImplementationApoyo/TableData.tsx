					...
					<Col>
						...
						>
							<DropdownToggle color="secondary dropdown-toggle-split">
								Buscar por {selectedColumn.Header?.toLowerCase()}{' '}
	...
	return (
		<Row>
			<Col className="container-nav">
				<Search onSearch={onChange} placeholderText={placeholderText} />
			</Col>
			{preferences && (
				<>
					<Col>
				...
				const filtered = rws.filter(row => {
					...
					for (const col of columns) {
						const normalizedRowValue = String(values[col])
							...
							.toLowerCase()
						valuesStr +=
							typeof values[col] === 'string' || typeof values[col] === 'number'
								? normalizedRowValue
								: ''
					}
			...
			<GlobalFilter
				...
				setGlobalFilter={value => {
					if (props.backendSearch) {
						return props.handleGetData(value, props.selectedColumn.column)
					}
			...
			{page.length === 0 && (
				...
				>
					{props.mensajeSinRegistros ? (
						...
						<p style={{ fontSize: 14 }}>
							{t('general>no_registros', 'No hay registros')}
						</p>
					)}
				</div>
			)}
...
export const TableDataBackendPaginated: React.FC<
	IBackendPaginatedProps
> = props => {
	const [columnsToShow, setColumnsToShow] = React.useState(
		props.columns.map(el => el.column)
	)
